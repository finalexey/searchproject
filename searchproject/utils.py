...
def send_request(word, top, skip):
...
def search(word, limit):
    ...
    """Функция поиска, принимающая аргументами искомое слово и количество необходимых результатов в выдаче"""
    ...
    if results_count > top:
        while top * count <= results_count and top > 0 and top_result <= results_count:
            if results_count - top_result >= top:
                search_result += send_request(word, top, skip)['results']
                skip = count * 10
                top_result += top
                count += 1
            else:
